...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_COUNTRY AS
    ...
    FUNCTION CHECK_COUNTRY_EXISTS 
        ...
        BEGIN
            ...
            select count(COUNTRY_NAME) into DB_COUNTRY_NAME from VMS_EMPLOYEE.COUNTRY where COUNTRY_NAME = vCOUNTRY_NAME;
        ...
        BEGIN
            ...
            IF VMS_EMPLOYEE.PCKG_COUNTRY.CHECK_COUNTRY_EXISTS(vCOUNTRY_NAME) = 'NO' then 
                INSERT INTO VMS_EMPLOYEE.COUNTRY(COUNTRY_ID, COUNTRY_NAME) VALUES (COUNTRY_ID_SEQ.NEXTVAL, vCOUNTRY_NAME);
            END IF; 
            
            IF VMS_EMPLOYEE.PCKG_COUNTRY.CHECK_COUNTRY_EXISTS(vCOUNTRY_NAME) = 'YES' then
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_STATE AS
    ...
    BEGIN
        select user into DBUser from dual;
        select count(STATE_NAME) into DB_STATE_NAME from VMS_EMPLOYEE.STATE where STATE_NAME = vSTATE_NAME;
    ...
    BEGIN
        ...
        BEGIN
        select user into DBUser from dual;
        select count(COUNTRY_NAME) into DB_COUNTRY_NAME_EXIST from VMS_EMPLOYEE.COUNTRY where COUNTRY_NAME = vCOUNTRY_NAME;
        EXCEPTION
        ...
        BEGIN
        select COUNTRY_ID into gCOUNTRY_ID from VMS_EMPLOYEE.COUNTRY where COUNTRY_NAME = vCOUNTRY_NAME;
         EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        if DB_COUNTRY_NAME_EXIST != 0 AND VMS_EMPLOYEE.PCKG_STATE.STATE_CHECK(vSTATE_NAME) = 'YES' then
            INSERT INTO VMS_EMPLOYEE.STATE(STATE_ID, STATE_NAME, COUNTRY_ID) VALUES (STATE_ID_SEQ.NEXTVAL, vSTATE_NAME, gCOUNTRY_ID);
        end if; 
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_CITY AS
    ...
    BEGIN
        select user into DBUser from dual;
        select count(CITY_NAME) into DB_CITY_NAME from VMS_EMPLOYEE.CITY where CITY_NAME = vCITY_NAME;
    ...
    BEGIN
        ...
        BEGIN
        select user into DBUser from dual;
        select count(STATE_NAME) into DB_STATE_NAME_EXIST from VMS_EMPLOYEE.STATE where STATE_NAME = vSTATE_NAME;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        BEGIN
        select STATE_ID into gSTATE_ID from VMS_EMPLOYEE.STATE where STATE_NAME = vSTATE_NAME;
         EXCEPTION
            WHEN NO_DATA_FOUND THEN
            gSTATE_ID := NULL;
        END;
        ...
        if DB_STATE_NAME_EXIST != 0 AND CITY_CHECK(vCITY_NAME) = 'YES' AND gSTATE_ID IS NOT NULL then
            INSERT INTO VMS_EMPLOYEE.CITY(CITY_ID, CITY_NAME, STATE_ID) VALUES (CITY_ID_SEQ.NEXTVAL, vCITY_NAME, gSTATE_ID);
        end if; 
...
create or replace PACKAGE BODY PKG_ADDRESS   AS
  ...
  END ADDRESS_VALIDATION;
    ...
    BEGIN
        select user into DBUser from dual;
        SELECT COUNT(*) INTO EXITSTING_CITY_NAME FROM VMS_EMPLOYEE.CITY WHERE CITY_NAME=vCITY_NAME;
        SELECT COUNT(*) INTO EXISTING_STATE_NAME FROM VMS_EMPLOYEE.STATE WHERE STATE_NAME=vSTATE_NAME;
        SELECT COUNT(*) INTO EXISTING_COUNTRY_NAME FROM VMS_EMPLOYEE.COUNTRY WHERE COUNTRY_NAME=vCOUNTRY_NAME;
        SELECT COUNT(*) INTO EXISTING_ADDRESS FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_LINE_1=vADDRESS_LINE_1;
        -- validate address
        IF VMS_EMPLOYEE.PKG_ADDRESS.ADDRESS_VALIDATION(vADDRESS_LINE_1, vADDRESS_LINE_2, vADDRESS_TYPE, vZIP_CODE, vCITY_NAME, vSTATE_NAME, vCOUNTRY_NAME) = 'NO' THEN
        ...
        IF EXISTING_COUNTRY_NAME = 0 THEN
            ...
            IF VMS_EMPLOYEE.PCKG_COUNTRY.CHECK_COUNTRY_EXISTS(UPPER(TRIM(vCOUNTRY_NAME))) = 'NO' THEN
            INSERT INTO VMS_EMPLOYEE.COUNTRY VALUES (
                FOREIGN_KEY_COUNTRY_ID,
                upper(trim(vCOUNTRY_NAME))
            );
            END IF;
        else    
            select COUNTRY_ID into FOREIGN_KEY_COUNTRY_ID from VMS_EMPLOYEE.COUNTRY where COUNTRY_NAME = vCOUNTRY_NAME;
        END IF;
        ...
        IF EXISTING_STATE_NAME = 0 THEN
            ...
            IF VMS_EMPLOYEE.PCKG_STATE.STATE_CHECK(UPPER(TRIM(vSTATE_NAME))) = 'NO' THEN
                RAISE INVALID_STATE_EX;
            END IF;
        ...
        ELSE
            select STATE_ID into FOREIGN_KEY_STATE_ID from VMS_EMPLOYEE.STATE where STATE_NAME = vSTATE_NAME;
        END IF;
        ...
        IF NOT EXITSTING_CITY_NAME = 0 THEN
            ...
            IF VMS_EMPLOYEE.PCKG_CITY.CITY_CHECK(UPPER(TRIM(vCITY_NAME))) = 'NO' THEN
                RAISE INVALID_CITY_EX;
            END IF;
            INSERT INTO VMS_EMPLOYEE.CITY VALUES (
                 FOREIGN_KEY_CITY_ID,
        ...
        INSERT INTO VMS_EMPLOYEE.ADDRESS VALUES(
    ...
    BEGIN
        select user into DBUser from dual;
        IF NOT vADDRESS_LINE_1 IS NULL or NOT LENGTH(trim(vADDRESS_LINE_1)) IS NULL then
            UPDATE VMS_EMPLOYEE.ADDRESS SET ADDRESS_LINE_1=vADDRESS_LINE_1 WHERE ADDRESS_ID=vADDRESS_ID;
        END IF;

        IF NOT vADDRESS_LINE_2 IS NULL or NOT LENGTH(trim(vADDRESS_LINE_2)) IS NULL then
            UPDATE VMS_EMPLOYEE.ADDRESS SET ADDRESS_LINE_2=vADDRESS_LINE_2 WHERE ADDRESS_ID=vADDRESS_ID;
        END IF;

        IF NOT vZIP_CODE IS NULL or NOT LENGTH(trim(vZIP_CODE)) IS NULL then
            UPDATE VMS_EMPLOYEE.ADDRESS SET ZIPCODE=vZIP_CODE WHERE ADDRESS_ID=vADDRESS_ID;
        END IF;

        IF NOT vADDRESS_TYPE IS NULL or NOT LENGTH(trim(vADDRESS_TYPE)) IS NULL then
            UPDATE VMS_EMPLOYEE.ADDRESS SET ADDRESS_TYPE=vADDRESS_TYPE WHERE ADDRESS_ID=vADDRESS_ID;
        END IF;

        IF NOT vCITY_NAME IS NULL or NOT LENGTH(trim(vCITY_NAME)) IS NULL then
            SELECT CITY_ID INTO FOREIGN_CITY_ID FROM VMS_EMPLOYEE.CITY WHERE CITY_ID IN 
            (SELECT CITY_ID FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_ID=vADDRESS_ID);
            UPDATE VMS_EMPLOYEE.STATE SET STATE_NAME=vSTATE_NAME WHERE STATE_ID=FOREIGN_STATE_ID;
        END IF;

        IF NOT vSTATE_NAME IS NULL or NOT LENGTH(trim(vSTATE_NAME)) IS NULL then
            UPDATE VMS_EMPLOYEE.STATE SET STATE_NAME=vSTATE_NAME WHERE STATE_ID IN
            (SELECT STATE_ID FROM VMS_EMPLOYEE.CITY WHERE CITY_ID IN
            (SELECT CITY_ID FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_ID=vADDRESS_ID));
        END IF;

        IF  NOT vCOUNTRY_NAME IS NULL or NOT LENGTH(trim(vCOUNTRY_NAME)) IS NULL then  
            UPDATE VMS_EMPLOYEE.COUNTRY SET COUNTRY_NAME=vCOUNTRY_NAME WHERE COUNTRY_ID IN
            (SELECT COUNTRY_ID FROM VMS_EMPLOYEE.STATE WHERE STATE_ID IN
            (SELECT STATE_ID FROM VMS_EMPLOYEE.CITY WHERE CITY_ID IN
            (SELECT CITY_ID FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_ID=vADDRESS_ID)));
        END IF;
    END UPDATE_ADDRESS;
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_ROLE  AS 
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
        select COUNT(ROLE_NAME) INTO DB_ROLE_NAME FROM VMS_EMPLOYEE.ROLE WHERE ROLE_NAME = vROLE_NAME;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
            DB_ROLE_NAME := 0;
        END;
        ...
        IF DB_ROLE_NAME = 0 AND UPPER(vROLE_NAME) in ('OWNER','DEVELOPER','TESTER', 'ANALYST') THEN
            INSERT INTO VMS_EMPLOYEE.ROLE(ROLE_ID, ROLE_NAME) VALUES ('ROLE_'||ROLE_ID_SEQ.NEXTVAL, vROLE_NAME);
        END IF; 
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_EMPLOYEE AS
    ...
    PROCEDURE INSERT_EMPLOYEE(
        ...
        BEGIN
            ...
            BEGIN
                SELECT count(*) INTO vCount FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_TYPE='EMPLOYEE' and ADDRESS_ID= UPPER(TRIM(vADDRESS_ID));
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        vCount := 0;
            END;
            ...
            END IF;
            BEGIN
                SELECT count(*) INTO vCount2 FROM VMS_EMPLOYEE.ROLE WHERE ROLE_ID = UPPER(TRIM(vROLE_ID));
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        vCount2 := 0;
            END;
            ...
            END IF;
            BEGIN
                SELECT count(*) INTO vCount3 FROM VMS_EMPLOYEE.EMPLOYEE WHERE EMP_EMAIL = UPPER(TRIM(vEMP_EMAIL));
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        vCount3 := 0;
            END;
            ...
            END IF;
            INSERT INTO VMS_EMPLOYEE.EMPLOYEE(EMPLOYEE_ID, EMP_FIRST_NAME, EMP_LAST_NAME, EMP_EMAIL, EMP_PASSWORD, DATE_OF_JOINING, ADDRESS_ID, ROLE_ID)
            VALUES
    ...
    PROCEDURE REMOVE_EMPLOYEE(
        ...
        select count(*) INTO vUSER_COUNT from VMS_EMPLOYEE.EMPLOYEE where EMP_EMAIL = UPPER(TRIM(vEMP_EMAIL));
        IF vUSER_COUNT = 0 THEN
            RAISE ex_EMPLOYEE_DONT_EXIST;
        END IF;
        SELECT EMPLOYEE_ID INTO vEMP_ID FROM VMS_EMPLOYEE.EMPLOYEE WHERE EMP_EMAIL = UPPER(TRIM(vEMP_EMAIL));
        SELECT COUNT(*) INTO vREGISTRATIONS_COUNT FROM VMS_EMPLOYEE.CAR_REGISTRATION WHERE EMPLOYEE_ID = vEMP_ID;
        ...
        DELETE FROM VMS_EMPLOYEE.EMPLOYEE WHERE UPPER(EMP_EMAIL) = UPPER(TRIM(vEMP_EMAIL));
    EXCEPTION
...
CREATE OR REPLACE EDITIONABLE PACKAGE  BODY PCKG_CAR_CATEGORY   AS
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
            select count(CAR_CATEGORY_ID) INTO vCount from VMS_EMPLOYEE.CAR_CATEGORY where CAR_CATEGORY_NAME = UPPER(trim(vCAR_CATEGORY_NAME));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount := 0;
        END;
        IF vCount > 0 THEN
            raise ex_EXISTS;
        END IF;
        IF VMS_EMPLOYEE.PCKG_CAR_CATEGORY.PROCESS_CAR_CATEGORY(UPPER(trim(vCAR_CATEGORY_NAME))) = 'NO' THEN
            raise ex_INVALID;
        END IF;
        SELECT 'CAR_CATEGORY_'||CAR_CATEGORY_ID_SEQ.NEXTVAL INTO vCAR_CATEGORY_ID FROM DUAL;
        INSERT INTO VMS_EMPLOYEE.CAR_CATEGORY(CAR_CATEGORY_ID, CAR_CATEGORY_NAME)
        VALUES(
            vCAR_CATEGORY_ID,
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
            select count(CAR_CATEGORY_ID) INTO vCount FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE CAR_CATEGORY_ID = vCAR_CATEGORY_ID;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount := 0;
        END;
        ...
        END IF;
        BEGIN
            select count(CAR_CATEGORY_ID) INTO vCount2 from VMS_EMPLOYEE.CAR_CATEGORY where CAR_CATEGORY_NAME = UPPER(trim(newCATEGORY_NAME));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount2 := 0;
        END;
        IF vCount2 > 0 THEN
            RAISE ex_EXISTS;
        END IF;
        IF VMS_EMPLOYEE.PCKG_CAR_CATEGORY.PROCESS_CAR_CATEGORY(UPPER(trim(newCATEGORY_NAME))) = 'NO' THEN
            raise ex_INVALID;
        END IF;
        UPDATE VMS_EMPLOYEE.CAR_CATEGORY SET CAR_CATEGORY_NAME = UPPER(trim(newCATEGORY_NAME)) WHERE CAR_CATEGORY_ID = vCAR_CATEGORY_ID;
    EXCEPTION
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
            SELECT count(*) INTO vCount FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE CAR_CATEGORY_NAME = UPPER(trim(vCAR_CATEGORY_NAME));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount := 0;
        END;
        IF vCount != 1 THEN
            BEGIN
                VMS_EMPLOYEE.PCKG_CAR_CATEGORY.INSERT_CAR_CATEGORY(vCAR_CATEGORY_NAME);
            END;
        END IF;
        SELECT CAR_CATEGORY_ID INTO vCAR_CATEGORY_ID FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE CAR_CATEGORY_NAME = UPPER(trim(vCAR_CATEGORY_NAME));
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
            SELECT COUNT(*) INTO vCAR_CATEGORY_COUNT FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE UPPER(CAR_CATEGORY_NAME) = UPPER(vCAR_CATEGORY_NAME);
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
        ...
        ELSE
            SELECT CAR_CATEGORY_ID INTO vCAR_CATEGORY_ID FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE UPPER(CAR_CATEGORY_NAME) = UPPER(vCAR_CATEGORY_NAME);
            BEGIN
                SELECT COUNT(*) INTO vREGISTERED_CAR_COUNT FROM VMS_EMPLOYEE.CAR_REGISTRATION WHERE UPPER(CAR_CATEGORY_ID) = UPPER(vCAR_CATEGORY_ID);
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
            ...
            IF vREGISTERED_CAR_COUNT = 0 THEN
                ...
                DELETE FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE CAR_CATEGORY_ID = UPPER(vCAR_CATEGORY_ID);
            ELSE
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_PARKING AS
    ...
    PROCEDURE INSERT_PARKING(
        ...
        BEGIN
            select user into DBUser from dual;
            BEGIN
                SELECT COUNT(*) INTO vCount FROM VMS_EMPLOYEE.ADDRESS WHERE ADDRESS_ID = TRIM(vADDRESS_ID) and ADDRESS_TYPE = 'PARKING';
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        vCount := 0;
            END;
            ...
            END IF;
            BEGIN
                SELECT COUNT(*) INTO vCount2 FROM VMS_EMPLOYEE.PARKING WHERE ADDRESS_ID = vADDRESS_ID;
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        vCount2 := 0;
            END;
            ...
            END IF;
            IF vCount2 < 1 THEN
                INSERT INTO VMS_EMPLOYEE.PARKING(PARKING_ID,ADDRESS_ID)
                VALUES (
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_PARKING_EXT AS
    ...
    BEGIN
        ...
        BEGIN
            SELECT COUNT(*) INTO EXISTING_ADDRESS_COUNT FROM VMS_EMPLOYEE.ADDRESS WHERE UPPER(ADDRESS_LINE_1) = UPPER(TRIM(vADDRESS_LINE_1)) AND ADDRESS_TYPE = 'PARKING';
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
        ...
        IF EXISTING_ADDRESS_COUNT < 1 THEN
            ...
            VMS_EMPLOYEE.PKG_ADDRESS.INSERT_ADDRESS(UPPER(vADDRESS_LINE_1), UPPER(vADDRESS_LINE_2), vZIP_CODE, UPPER(vCITY_NAME), UPPER(vSTATE_NAME), 'PARKING', UPPER(vCOUNTRY_NAME));
        END IF;
        BEGIN
            SELECT COUNT(*) INTO EXISTING_ADDRESS_COUNT FROM VMS_EMPLOYEE.ADDRESS WHERE UPPER(ADDRESS_LINE_1) = UPPER(TRIM(vADDRESS_LINE_1)) AND ADDRESS_TYPE = 'PARKING';
            EXCEPTION
        ...
        IF EXISTING_ADDRESS_COUNT > 0 THEN
            SELECT ADDRESS_ID INTO vADDRESS_ID FROM VMS_EMPLOYEE.ADDRESS WHERE UPPER(ADDRESS_LINE_1) = UPPER(TRIM(vADDRESS_LINE_1)) AND ADDRESS_TYPE = 'PARKING';
            -- TRY INSERTING PARKING
            VMS_EMPLOYEE.PCKG_PARKING.INSERT_PARKING(vADDRESS_ID);
        ELSE
...
create or replace PACKAGE BODY PKG_CAR_REGISTER AS
    ...
    BEGIN
        ...
        if vEMP_EMAIL is NULL or LENGTH(TRIM(vEMP_EMAIL)) is NULL or NOT (TRIM(vEMP_EMAIL)) LIKE '%@_%._%' then
         ...
         begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
            IF ID_CHECK > 0 THEN
                raise INVALID_CAR_REGISTER_ID_PRESENT;
            END IF;
        end;
        begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.EMPLOYEE where EMP_EMAIL = TRIM(UPPER(vEMP_EMAIL));
            if(ID_CHECK = 0) then 
                raise INVALID_EMPLOYEE_ID;
            end if;
            select EMPLOYEE_ID into vEMPLOYEE_ID from VMS_EMPLOYEE.EMPLOYEE where EMP_EMAIL = TRIM(UPPER(vEMP_EMAIL));
        end;
        begin
        ...
        begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.Address where ADDRESS_LINE_1 = TRIM(UPPER(vADDRESS_LINE_1)) and ADDRESS_TYPE = 'PARKING';
            if(ID_CHECK = 0) then  
                VMS_EMPLOYEE.PCKG_PARKING_EXT.INSERT_PARKING_EXT(vADDRESS_LINE_1, vADDRESS_LINE_2, vZIP_CODE, vCITY_NAME, vSTATE_NAME, vCOUNTRY_NAME);
                select ADDRESS_ID into vADDRESS_ID from VMS_EMPLOYEE.Address where ADDRESS_LINE_1 = TRIM(UPPER(vADDRESS_LINE_1)) and ADDRESS_TYPE = 'PARKING';
            else
                select ADDRESS_ID into vADDRESS_ID from VMS_EMPLOYEE.Address where ADDRESS_LINE_1 = TRIM(UPPER(vADDRESS_LINE_1)) and ADDRESS_TYPE = 'PARKING';
            end if;
        end;
        ...
        VMS_EMPLOYEE.PKG_CAR_REGISTER.BODY_CAR_REGISTER(UPPER(vCAR_REGISTER_ID), UPPER(vCAR_NAME), UPPER(vCAR_COLOR), UPPER(vCAR_COMPANY), to_number(vYEAR_OF_MANUFACTURE), vREGISTRATION_DATE, UPPER(vFUEL_TYPE), UPPER(vTRANSMISSION_TYPE), to_number(vNO_OF_SEATS), UPPER(vCAR_CATEGORY_ID), UPPER(vEMPLOYEE_ID), UPPER(vADDRESS_ID));
        begin
            dbms_output.put_line(vADDRESS_ID);
            select PARKING_ID into vCAR_PARKING_ID from VMS_EMPLOYEE.Parking where ADDRESS_ID = TRIM(UPPER(vADDRESS_ID));
        end;   
        VMS_EMPLOYEE.PCKG_CAR_LISTING.INSERT_CAR_LISTING('TRUE',vFEE_RATE,TRIM(UPPER(vCAR_REGISTER_ID)),TRIM(UPPER(vCAR_PARKING_ID)));
    EXCEPTION
    ...
    BEGIN     
        ...
        begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.CAR_CATEGORY where CAR_CATEGORY_ID = TRIM(UPPER(vCAR_CATEGORY_ID));
            if(ID_CHECK = 0) then 
                raise INVALID_CAR_CATEGORY_ID;
            end if;
        end;
        ...
        begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.EMPLOYEE where EMPLOYEE_ID = TRIM(UPPER(vEMPLOYEE_ID));
            if(ID_CHECK = 0) then 
                raise INVALID_EMPLOYEE_ID;
            end if;
        end;
        ...
        begin
            select Count(*) into ID_CHECK from VMS_EMPLOYEE.ADDRESS where ADDRESS_ID = TRIM(UPPER(vADDRESS_ID)) AND ADDRESS_TYPE = 'PARKING';
             if(ID_CHECK = 0) then 
                raise INVALID_ADDRESS_ID;
            end if;
        end;
    ...
    BEGIN
        select user into DBUser from dual;
        select Count(*) into counts FROM VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        RETURN counts;
    ...
    BEGIN
        ...
        select listing_id into vlisting_id from VMS_EMPLOYEE.car_listing where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        select address_id into vaddress_id from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        select count(*) into vbooking_count from VMS_EMPLOYEE.booking where listing_id = TRIM(UPPER(vlisting_id)) and (BOOKING_STATUS in ('INITIAL', 'IN-PROGRESS'));
        if vbooking_count = 0 then
            Delete from  VMS_EMPLOYEE.car_listing where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
            Delete from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
            Delete from  VMS_EMPLOYEE.parking where address_id = TRIM(UPPER(vaddress_id));
            Delete from  VMS_EMPLOYEE.address where address_id = TRIM(UPPER(vaddress_id));
    ...
    BEGIN
        ...
        Select VMS_EMPLOYEE.PKG_CAR_REGISTER.BODY_CAR_REGISTER_ID_CHECK(vCAR_REGISTER_ID) INTO temp_car_id from dual;
        if Not(temp_car_id > 0) then
             raise INVALID_CAR_REGISTER_ID;
        end if;
        Select ADDRESS_ID into vADDRESS_ID from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        if uCAR_CATEGORY_NAME is NULL or LENGTH(TRIM(uCAR_CATEGORY_NAME)) is NULL then
            Select CAR_CATEGORY_ID into vCAR_CATEGORY_ID from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            begin
            VMS_EMPLOYEE.PCKG_CAR_CATEGORY.CHECK_AND_RETURN_ID(uCAR_CATEGORY_NAME, vCAR_CATEGORY_ID);
        end;
        end if;
        
        if vEMP_EMAIL is NULL or LENGTH(TRIM(vEMP_EMAIL)) is NULL then
            Select EMPLOYEE_ID into vEMPLOYEE_ID from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            if vEMP_EMAIL is NULL or LENGTH(TRIM(vEMP_EMAIL)) is NULL or NOT (TRIM(vEMP_EMAIL)) LIKE '%@_%._%' then
                raise INVALID_EMPLOYEE_ID;
            end if;
        ...
        if uCAR_NAME is NULL or LENGTH(TRIM(uCAR_NAME)) is NULL then
            Select CAR_NAME into vCAR_NAME from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vCAR_NAME:= uCAR_NAME;
        ...
        if uCAR_COLOR is NULL or  LENGTH(uCAR_COLOR) is NULL then
            Select CAR_COLOR into vCAR_COLOR from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vCAR_COLOR:= uCAR_COLOR;
        end if;

        if uCAR_COMPANY is NULL or LENGTH(TRIM(uCAR_COMPANY)) is NULL then
            Select CAR_COMPANY into vCAR_COMPANY from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vCAR_COMPANY:= uCAR_COMPANY;
        end if; 

        if uYEAR_OF_MANUFACTURE is NULL or to_char(LENGTH(uYEAR_OF_MANUFACTURE)) is NULL then
            Select YEAR_OF_MANUFACTURE into vYEAR_OF_MANUFACTURE from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vYEAR_OF_MANUFACTURE:= uYEAR_OF_MANUFACTURE;
        end if;
        
        if uFUEL_TYPE is NULL or LENGTH(TRIM(uFUEL_TYPE)) is NULL then
            Select FUEL_TYPE into vFUEL_TYPE from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vFUEL_TYPE:= uFUEL_TYPE;
        end if;  

        if uTRANSMISSION_TYPE is NULL or  LENGTH(uTRANSMISSION_TYPE) is NULL then
            Select TRANSMISSION_TYPE into vTRANSMISSION_TYPE from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        else
            vTRANSMISSION_TYPE:= uTRANSMISSION_TYPE;
        end if;

        if uNO_OF_SEATS is NULL or  LENGTH(uNO_OF_SEATS) = 0 then
            Select NO_OF_SEATS into vNO_OF_SEATS from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
            vNO_OF_SEATS:= uNO_OF_SEATS;
        end if;

        if uREGISTRATION_DATE is NULL or to_char(LENGTH(uREGISTRATION_DATE)) is NULL then
                Select REGISTRATION_DATE into vREGISTRATION_DATE from VMS_EMPLOYEE.CAR_REGISTRATION where CAR_REGISTER_ID = TRIM(UPPER(vCAR_REGISTER_ID));
        else
        ...
        if Conditions = 'YES' and  temp_car_id > 0 THEN
            UPDATE VMS_EMPLOYEE.CAR_REGISTRATION SET CAR_NAME = UPPER(vCAR_NAME),  
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_CAR_LISTING AS
    ...
    BEGIN
        ...
        END IF;
        BEGIN
            SELECT count(*) INTO CAR_REGISTER_COUNT FROM VMS_EMPLOYEE.CAR_REGISTRATION WHERE CAR_REGISTER_ID = UPPER(TRIM(vCAR_REGISTER_ID));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
        ...
        END IF;
        BEGIN
            SELECT count(*) INTO vCount FROM VMS_EMPLOYEE.PARKING WHERE PARKING_ID = UPPER(TRIM(vCAR_PARKING_ID));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount := 0;
        END;
        ...
        END IF;
        BEGIN
            SELECT count(*) INTO CAR_PARKING_COUNT FROM VMS_EMPLOYEE.CAR_LISTING WHERE PARKING_ID = UPPER(TRIM(vCAR_PARKING_ID));
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    CAR_PARKING_COUNT := 0;
        END;
    ...
    BEGIN
        select user into DBUser from dual;
        IF VMS_EMPLOYEE.PCKG_CAR_LISTING.PROCESS_CAR_LISTING(vAVAILABILITY, SYSDATE,vFEE_RATE,vCAR_REGISTER_ID, vCAR_PARKING_ID) = 'NO' THEN
            RAISE ex_INVALID;
        END IF;
        INSERT INTO VMS_EMPLOYEE.CAR_LISTING(LISTING_ID, AVAILABILITY, DATE_POSTED, FEE_RATE, CAR_REGISTER_ID, PARKING_ID)
            VALUES(
    ...
    BEGIN
        ...
        BEGIN
            SELECT count(*) INTO vCount FROM VMS_EMPLOYEE.CAR_LISTING WHERE LISTING_ID = vCAR_LISTING_ID;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    vCount := 0;
        END;
        ...
        UPDATE VMS_EMPLOYEE.CAR_LISTING SET FEE_RATE = vFEE_RATE WHERE LISTING_ID = vCAR_LISTING_ID;
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PKG_TRANSACTION AS
    ...
    BEGIN
        select user into DBUser from dual;
        vTRANSACTION_ID:= VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_ID_GENERATE(vSTATUS, vTOTAL_COST);
        VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_INSERT(vTRANSACTION_ID, vSTATUS, vTOTAL_COST);
    ...
    BEGIN
        select user into DBUser from dual;
        Select VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_CHECK(vSTATUS, vTOTAL_COST) INTO Conditions from dual;
        if Conditions = 'YES' THEN
             VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_INSERT_TABLE(vTRANSACTION_ID, UPPER(vSTATUS), vTOTAL_COST);
             dbms_output.put_line('Data inserted successfully !!!');
        end if;
    ...
    BEGIN
        ...
        INSERT INTO VMS_EMPLOYEE.TRANSACTION (TRANSACTION_ID, STATUS, TOTAL_COST) VALUES (vTRANSACTION_ID, vSTATUS, vTOTAL_COST);
    ...
    BEGIN   
        select user into DBUser from dual;
        Select VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_ID_CHECK(vTRANSACTION_ID) INTO temp_trans_id from dual;
        if temp_trans_id > 0 THEN
            UPDATE VMS_EMPLOYEE.TRANSACTION SET STATUS = UPPER(vSTATUS) WHERE TRANSACTION_ID = vTRANSACTION_ID;
            dbms_output.put_line('Data updated successfully !!!');
        ELSE
    ...
    BEGIN
        ...
        if Conditions = 'YES' and  temp_trans_id > 0 THEN
            UPDATE VMS_EMPLOYEE.TRANSACTION SET STATUS = UPPER(vSTATUS),  TOTAL_COST = vTOTAL_COST WHERE TRANSACTION_ID = vTRANSACTION_ID;
            dbms_output.put_line('Data updated successfully !!!');
        ELSE
            dbms_output.put_line('Invalid TRANSACTION ID !!!');
         end if;
    ...
    BEGIN
        select user into DBUser from dual;
        begin
        select Count(*) into counts FROM VMS_EMPLOYEE.TRANSACTION where TRANSACTION_ID = vTRANSACTION_ID;
        RETURN counts;
        exception 
            when NO_DATA_FOUND THEN 
                counts := 0;
...
create or replace PACKAGE BODY PKG_BOOKING   AS
    ...
    FUNCTION NEW_BOOKING_VALIDATION(
        ...
        BEGIN
            ...
            SELECT COUNT(*) INTO USER_EXISTS FROM VMS_EMPLOYEE.USERS WHERE USER_ID=UPPER(TRIM(vUSER_ID));
            SELECT COUNT(*) INTO LISTING_EXISTS FROM VMS_EMPLOYEE.CAR_LISTING WHERE LISTING_ID=UPPER(TRIM(vLISTING_ID));
    ...
    FUNCTION BOOKING_IN_PROGRESS_VALIDATION(
        ...
        BEGIN
            SELECT COUNT(*) INTO BOOKING_STATUS_INPROG FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_STATUS='IN-PROGRESS' AND BOOKING_ID=vBOOKING_ID;
            SELECT COUNT(*) INTO BOOKING_STATUS_COMP FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_STATUS='COMPLETED' AND BOOKING_ID=vBOOKING_ID;
            SELECT COUNT(*) INTO vIS_BOOKING_PRESENT FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
    ...
    FUNCTION BOOKING_COMPLETED_VALIDATION(
        ...
        BEGIN
            SELECT COUNT(*) INTO BOOKING_STATUS_INITIAL FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_STATUS='INITIAL' AND BOOKING_ID=vBOOKING_ID;
            SELECT COUNT(*) INTO BOOKING_STATUS_COMP FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_STATUS='COMPLETED' AND BOOKING_ID=vBOOKING_ID;
    ...
    PROCEDURE INSERT_NEW_BOOKING(
        ...
        BEGIN
            SELECT COUNT(*) INTO EXISTING_BOOKING FROM VMS_EMPLOYEE.BOOKING WHERE USER_ID=vUSER_ID AND BOOKING_STATUS IN ('INITIAL', 'IN-PROGRESS', 'COMPLETED', 'PAYMENT_FAILED');
            ...
            -- validate address
        IF VMS_EMPLOYEE.PKG_BOOKING.NEW_BOOKING_VALIDATION(vCREATED_START_DATE, vCREATED_END_DATE, vUSER_ID, vLISTING_ID) = 'NO' THEN
            RAISE INVALID_INPUT_EX;
        END IF;
            ...
            SELECT FEE_RATE INTO vFEE_RATE FROM VMS_EMPLOYEE.CAR_LISTING WHERE LISTING_ID=vLISTING_ID;

            -- GET THE VALUE OF LISTING_IF FROM CAR_REGISTRATION ENTITY
            VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION('PENDING', vDAYS * vFEE_RATE, FETCH_TRANSACTION_ID);

            INSERT INTO VMS_EMPLOYEE.BOOKING(BOOKING_ID, BOOKING_STATUS, CREATED_START_DATE, CREATED_END_DATE, USER_ID,TRANSACTION_ID,LISTING_ID) VALUES(
    ...
    PROCEDURE BOOKING_IN_PROGRESS(
        ...
        BEGIN
        -- validate BOOKING
        IF VMS_EMPLOYEE.PKG_BOOKING.BOOKING_IN_PROGRESS_VALIDATION(vBOOKING_ID,vLISTING_ID) = 'NO' THEN
            RAISE INVALID_INPUT_EX;
        END IF;

        -- UPDATE THE BOOKING STATUS
        UPDATE VMS_EMPLOYEE.BOOKING SET BOOKING_STATUS='IN-PROGRESS', ACTUAL_START_DATE=LOCALTIMESTAMP WHERE BOOKING_ID=vBOOKING_ID;
    ...
    PROCEDURE BOOKING_COMPLETED(
        ...
        BEGIN
            -- validate BOOKING
        IF VMS_EMPLOYEE.PKG_BOOKING.BOOKING_COMPLETED_VALIDATION(vBOOKING_ID, vLISTING_ID) = 'NO' THEN
            RAISE INVALID_INPUT_EX;
        END IF;

        BEGIN
        SELECT ACTUAL_START_DATE INTO vACTUAL_START_DATE FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
				EXCEPTION
					WHEN NO_DATA_FOUND THEN
					RAISE NO_DATA_EX;
        END;
        ...
        BEGIN
        SELECT TRANSACTION_ID INTO vTRANSACTION_ID FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
        				EXCEPTION
					WHEN NO_DATA_FOUND THEN
					RAISE NO_DATA_EX;
        END;

        VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_UPDATE_TABLE(vTRANSACTION_ID,'PENDING', vDAYS * vFEE_RATE);

        -- UPDATE THE BOOKING STATUS
        UPDATE VMS_EMPLOYEE.BOOKING SET BOOKING_STATUS='COMPLETED', COMMENTS=vCOMMENTS, ACTUAL_END_DATE=LOCALTIMESTAMP WHERE BOOKING_ID=vBOOKING_ID;
    ...
    PROCEDURE BOOKING_PAYMENT_SUCCESS(
        ...
        SELECT COUNT(*) TRANSACTION_ID INTO RECORD_COUNT FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
        SELECT COUNT(*) BOOKING_STATUS INTO PAID FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='PAYMENT_DONE';
        SELECT COUNT(*) BOOKING_STATUS INTO INITIAL1 FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='INITIAL';
        SELECT COUNT(*) BOOKING_STATUS INTO IN_PROGRESS FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='IN-PROGRESS';
        ...
        SELECT TRANSACTION_ID INTO vTRANSACTION_ID FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
					...
					RAISE NO_DATA_EX;
        END;
        VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_UPDATE_TABLE_SUCCESS(vTRANSACTION_ID, 'SUCCESS');

        -- UPDATE THE BOOKING STATUS
        UPDATE VMS_EMPLOYEE.BOOKING SET BOOKING_STATUS='PAYMENT_DONE', ACTUAL_END_DATE=LOCALTIMESTAMP WHERE BOOKING_ID=vBOOKING_ID;
    ...
    PROCEDURE BOOKING_PAYMENT_FAIL(
        ...
        SELECT COUNT(*) TRANSACTION_ID INTO RECORD_COUNT FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
        SELECT COUNT(*) BOOKING_STATUS INTO PAID FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='PAYMENT_FAILED';
        SELECT COUNT(*) BOOKING_STATUS INTO INITIAL1 FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='INITIAL';
        SELECT COUNT(*) BOOKING_STATUS INTO IN_PROGRESS FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='IN-PROGRESS';
        ...
        SELECT TRANSACTION_ID INTO vTRANSACTION_ID FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
					...
					RAISE NO_DATA_EX;
        END;
        VMS_EMPLOYEE.PKG_TRANSACTION.BODY_TRANSACTION_UPDATE_TABLE_SUCCESS(vTRANSACTION_ID, 'FAILED');

        -- UPDATE THE BOOKING STATUS
        UPDATE VMS_EMPLOYEE.BOOKING SET BOOKING_STATUS='PAYMENT_FAILED', ACTUAL_END_DATE=LOCALTIMESTAMP WHERE BOOKING_ID=vBOOKING_ID;
    ...
    PROCEDURE BOOKING_CANCELLED(
        ...
        BEGIN
            ...
            BEGIN
                SELECT CREATED_START_DATE INTO vCREATED_DATE_TIME FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID;
				EXCEPTION
					WHEN NO_DATA_FOUND THEN
					RAISE NO_DATA_EX;
            END;
            ...
            BEGIN
                SELECT COUNT(*) INTO CANCELLED_COUNT FROM VMS_EMPLOYEE.BOOKING WHERE BOOKING_ID=vBOOKING_ID AND BOOKING_STATUS='CANCELLED';
				EXCEPTION
					WHEN NO_DATA_FOUND THEN
					RAISE NO_DATA_EX;
            END;
            ...
            IF vCREATED_DATE_TIME > LOCALTIMESTAMP THEN
                UPDATE VMS_EMPLOYEE.BOOKING SET BOOKING_STATUS='CANCELLED' WHERE BOOKING_ID=vBOOKING_ID;
            ELSE
                RAISE CANNOT_CANCEL_EX;
            END IF;

            -- change the total cost of the user to 0 after cancellation
            UPDATE VMS_EMPLOYEE.TRANSACTION SET TOTAL_COST = 0.0, STATUS='CANCELLED' WHERE TRANSACTION_ID=vTRANSACTION_ID;
        EXCEPTION
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_USERS  AS 
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
        SELECT COUNT(EMAIL) INTO CHECK_EMAIL_COUNT FROM VMS_EMPLOYEE.USERS WHERE EMAIL = vEMAIL; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
            CHECK_EMAIL_COUNT := 0;
        END;
        ...
        BEGIN
        SELECT COUNT(USER_NAME) INTO CHECK_USER_NAME_COUNT FROM VMS_EMPLOYEE.USERS WHERE USER_NAME = vUSER_NAME; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        BEGIN
        SELECT COUNT(DRIVER_LICENSE) INTO CHECK_DL_COUNT FROM VMS_EMPLOYEE.USERS WHERE DRIVER_LICENSE = vDRIVER_LICENSE; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
            CHECK_DL_COUNT := 0;
        END;
        ...
        BEGIN
        SELECT COUNT(PASSPORT) INTO CHECK_PASSPORT_COUNT FROM VMS_EMPLOYEE.USERS WHERE PASSPORT = vPASSPORT; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        INSERT INTO VMS_EMPLOYEE.USERS(USER_ID, PASSWORD, USER_NAME, EMAIL, PHONE_NO, FIRST_NAME, LAST_NAME, DATE_OF_JOINING, DRIVER_LICENSE, PASSPORT, BLACKLISTED, ADDRESS_ID)
        VALUES
    ...
    BEGIN
        select user into DBUser from dual;
        BEGIN
        SELECT USER_ID INTO USER_ID_VAL_FROM_USER_TBL FROM VMS_EMPLOYEE.USERS WHERE EMAIL = vEMAIL; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        BEGIN
        SELECT COUNT(USER_ID) INTO USER_USER_ID_IN_BOOKING_COUNT FROM VMS_EMPLOYEE.BOOKING WHERE USER_ID = USER_ID_VAL_FROM_USER_TBL; 
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        IF USER_USER_ID_IN_BOOKING_COUNT = 0 THEN 
        DELETE FROM VMS_EMPLOYEE.USERS WHERE UPPER(EMAIL) = UPPER(vEMAIL);
        END IF; 
    ...
    PROCEDURE UPDATE_USER(
        ...
        BEGIN
            select user into DBUser from dual;
            BEGIN
            SELECT COUNT(DRIVER_LICENSE) INTO CHECK_DRIVER_LICENSE_COUNT FROM VMS_EMPLOYEE.USERS  WHERE DRIVER_LICENSE = vDRIVER_LICENSE;
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
        ...
        BEGIN
            select user into DBUser from dual;
            BEGIN
            SELECT COUNT(ADDRESS_ID) INTO CHECK_ADDRESS_ID FROM VMS_EMPLOYEE.ADDRESS  WHERE ADDRESS_ID = vADDRESS_ID;
            EXCEPTION
            WHEN NO_DATA_FOUND THEN
            CHECK_ADDRESS_ID := 0;
            END;
...
CREATE OR REPLACE EDITIONABLE PACKAGE BODY PCKG_CAR_LISTING_EXT AS
    ...
    BEGIN
        ...
        IF vUSER_ID IS NOT NULL THEN
            SELECT count(*) INTO vUSER_COUNT FROM VMS_EMPLOYEE.USERS WHERE USER_ID = UPPER(TRIM(vUSER_ID));
        ...
        SELECT count(*) INTO vBOOKINGS_COUNT FROM VMS_EMPLOYEE.BOOKING WHERE UPPER(BOOKING_STATUS) IN ('INITIAL','IN-PROGRESS','COMPLETED','PAYMENT_FAILED') AND UPPER(USER_ID) = UPPER(TRIM(vUSER_ID));
        IF vBOOKINGS_COUNT != 0 THEN
            RAISE PREV_BOOKING_PENDING;
        ELSE
            -- FILTER BASED ON PARAMETERS
            
            SELECT count(*) INTO vADDRESS_COUNT FROM VMS_EMPLOYEE.ADDRESS WHERE UPPER(ZIPCODE) = UPPER(TRIM(vZIP_CODE)) AND UPPER(ADDRESS_TYPE) = 'PARKING';
            ...
            DECLARE
                CURSOR V_CUR
                IS
                SELECT * FROM VMS_EMPLOYEE.CAR_LISTING
                WHERE
                PARKING_ID IN ( 
                    SELECT PARKING_ID FROM VMS_EMPLOYEE.PARKING 
                    WHERE 
                        ADDRESS_ID IN (
                            SELECT ADDRESS_ID FROM VMS_EMPLOYEE.ADDRESS WHERE UPPER(ZIPCODE) = UPPER(TRIM(vZIP_CODE)) AND UPPER(ADDRESS_TYPE)='PARKING'
                        )
                    )
                AND
                CAR_REGISTER_ID IN (
                    SELECT CAR_REGISTER_ID FROM VMS_EMPLOYEE.CAR_REGISTRATION
                    WHERE 
                        NO_OF_SEATS >= vMIN_NO_OF_SEATS
                        AND 
                        CAR_CATEGORY_ID IN (
                            SELECT CAR_CATEGORY_ID FROM VMS_EMPLOYEE.CAR_CATEGORY WHERE
                            CAR_CATEGORY_NAME = UPPER(TRIM(vCAR_CATEGORY_NAME))
                        )
                    )
                    AND
                    LISTING_ID NOT IN(
                        SELECT LISTING_ID FROM VMS_EMPLOYEE.BOOKING
                        WHERE
