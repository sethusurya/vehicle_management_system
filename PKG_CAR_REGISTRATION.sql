...
CREATE OR REPLACE EDITIONABLE PACKAGE PKG_CAR_REGISTER   AS 
    PROCEDURE BODY_CAR_REGISTER( 
        ...
        vYEAR_OF_MANUFACTURE IN CAR_REGISTRATION.YEAR_OF_MANUFACTURE%type,
        ...
        vNO_OF_SEATS IN CAR_REGISTRATION.NO_OF_SEATS%type,
    ...
    PROCEDURE BODY_CAR_REGISTER_UPDATE_TABLE(
        vCAR_REGISTER_ID IN CAR_REGISTRATION.CAR_REGISTER_ID%type,
        vCAR_NAME IN OUT CAR_REGISTRATION.CAR_NAME%type, 
        vCAR_COLOR IN OUT CAR_REGISTRATION.CAR_COLOR%type,
        vCAR_COMPANY IN OUT CAR_REGISTRATION.CAR_COMPANY%type,
        vYEAR_OF_MANUFACTURE IN OUT CAR_REGISTRATION.YEAR_OF_MANUFACTURE%type,
        vREGISTRATION_DATE IN OUT CAR_REGISTRATION.REGISTRATION_DATE%type,
        vFUEL_TYPE IN OUT CAR_REGISTRATION.FUEL_TYPE%type,
        vTRANSMISSION_TYPE IN OUT CAR_REGISTRATION.TRANSMISSION_TYPE%type,
        vNO_OF_SEATS IN OUT CAR_REGISTRATION.NO_OF_SEATS%type,
        vCAR_CATEGORY_ID IN CAR_REGISTRATION.CAR_CATEGORY_ID%type,
        vEMPLOYEE_ID IN CAR_REGISTRATION.EMPLOYEE_ID%type,
        vADDRESS_ID IN CAR_REGISTRATION.ADDRESS_ID%type
    );
...
create or replace PACKAGE BODY PKG_CAR_REGISTER AS
    ...
    BEGIN
        ...
        if Conditions = 'YES' THEN
             BODY_CAR_REGISTER_INSERT_TABLE(vCAR_REGISTER_ID, UPPER(vCAR_NAME), UPPER(vCAR_COLOR), UPPER(vCAR_COMPANY), vYEAR_OF_MANUFACTURE, vREGISTRATION_DATE, UPPER(vFUEL_TYPE), UPPER(vTRANSMISSION_TYPE), vNO_OF_SEATS, UPPER(vCAR_CATEGORY_ID), UPPER(vEMPLOYEE_ID), UPPER(vADDRESS_ID));
    ...
    BEGIN     
        ...
        if vYEAR_OF_MANUFACTURE is NULL or LENGTH(TRIM(vYEAR_OF_MANUFACTURE)) = 0 then
                raise INVALID_YEAR_OF_MANUFACTURE;
        end if;

        IF(NOT VALIDATE_CONVERSION(vYEAR_OF_MANUFACTURE AS NUMBER) = 1) THEN
            RAISE INVALID_YEAR_OF_MANUFACTURE;
        END IF;
        ...
        IF(NOT VALIDATE_CONVERSION(vNO_OF_SEATS AS NUMBER) = 1) THEN
            RAISE INVALID_NO_OF_SEATS;
        END IF;
    ...
    EXCEPTION
        ...
        when INVALID_YEAR_OF_MANUFACTURE then
            dbms_output.put_line('Invalid MANUFACTURE Date !!!');
            RETURN 'NO';
    ...
    counts NUMBER;
    BEGIN
        select Count(*) into counts FROM CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        RETURN counts;
    ...
    BEGIN
        ...
        if vCAR_NAME is NULL or LENGTH(TRIM(vCAR_NAME)) is NULL then
            Select CAR_NAME into vCAR_NAME from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;

        if vCAR_COLOR is NULL or  LENGTH(vCAR_COLOR) is NULL then
            Select CAR_COLOR into vCAR_COLOR from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;

        if vCAR_COMPANY is NULL or LENGTH(TRIM(vCAR_COMPANY)) is NULL then
            Select CAR_COMPANY into vCAR_COMPANY from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if; 

        if vYEAR_OF_MANUFACTURE is NULL or to_char(LENGTH(vYEAR_OF_MANUFACTURE)) is NULL then
            Select YEAR_OF_MANUFACTURE into vYEAR_OF_MANUFACTURE from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;

        if vFUEL_TYPE is NULL or LENGTH(TRIM(vFUEL_TYPE)) is NULL then
            Select FUEL_TYPE into vFUEL_TYPE from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;  

        if vTRANSMISSION_TYPE is NULL or  LENGTH(vTRANSMISSION_TYPE) is NULL then
            Select TRANSMISSION_TYPE into vTRANSMISSION_TYPE from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;

        if vNO_OF_SEATS is NULL or  LENGTH(vNO_OF_SEATS) = 0 then
            Select NO_OF_SEATS into vNO_OF_SEATS from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;

        if vREGISTRATION_DATE is NULL or to_char(LENGTH(vREGISTRATION_DATE)) is NULL then
                Select REGISTRATION_DATE into vREGISTRATION_DATE from CAR_REGISTRATION where vCAR_REGISTER_ID = UPPER(vCAR_REGISTER_ID);
        end if;
...
BEGIN
    PKG_CAR_REGISTER.BODY_CAR_REGISTER('A','B','C','23-APR-22','','','','','','','',returnvalue);
