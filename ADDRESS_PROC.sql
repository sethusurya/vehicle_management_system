...
create or replace PACKAGE BODY PKG_ADDRESS   AS
  ...
  FUNCTION ADDRESS_VALIDATION(
    ...
    BEGIN
        ...
        IF(NOT vADDRESS_TYPE = NULL AND NOT LENGTH(TRIM(vADDRESS_TYPE)) IS NULL) THEN
			RAISE ADDRESS_TYPE_EX;
		END IF;

        if UPPER(vADDRESS_TYPE) != 'PARKING' AND UPPER(vADDRESS_TYPE) != 'USER' AND UPPER(vADDRESS_TYPE) != 'EMPLOYEE' then
  ...
  END ADDRESS_VALIDATION;
    ...
    BEGIN
        ...
        -- validate address
        IF ADDRESS_VALIDATION(vADDRESS_LINE_1, vADDRESS_LINE_2, vZIP_CODE, vADDRESS_TYPE) = 'NO' THEN
